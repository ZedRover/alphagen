...
class Calculator10d(AlphaCalculator):
    def __init__(
        self,
        data: ArgData,
        ...
        self.raw_ret = np.load(DIR_RETS[3]).reshape(-1, N_PROD)
        self.ret = self.raw_ret[real_start_idx:real_end_idx]
        self.ret = torch.from_numpy(self.ret).to(device)
    ...
    def calc_single_IC_ret(self, expr: Expression) -> float:
        ...
        ic_1d = self._calc_IC(value, self.ret)
        return ic_1d
    ...
    def calc_pool_IC_ret(self, exprs: List[Expression], weights: List[float]) -> float:
        with torch.no_grad():
            ...
            ic = batch_pearsonr(ensemble_value, self.ret).mean().item()
            return ic

    def calc_pool_pIC_ret(self, exprs: List[Expression], weights: List[float]) -> float:
        with torch.no_grad():
            ensemble_value = self._make_ensemble_alpha(exprs, weights)
            pic = pool_pearsonr(ensemble_value, self.ret).item()
            return pic

    def calc_pool_rIC_ret(self, exprs: List[Expression], weights: List[float]) -> float:
        with torch.no_grad():
            ensemble_value = self._make_ensemble_alpha(exprs, weights)
            rank_ic = batch_spearmanr(ensemble_value, self.ret).mean().item()
            return rank_ic
    ...
    def calc_q90_ret(self, expr: Expression) -> float:
        value = self._calc_alpha(expr)
        return self._calc_q90(value, self.ret)
