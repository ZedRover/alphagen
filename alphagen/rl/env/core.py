...
class AlphaEnvCore(gym.Env):
    ...
    def step(self, action: Token) -> Tuple[List[Token], float, bool, bool, dict]:
        ...
        if (
            ...
            reward = self._evaluate() * 10 if self._builder.is_valid() else -1.0
    ...
    def _evaluate(self):
        ...
        try:
            ...
            print("Out of data range, skipping...")
            return 0.0
    ...
    def _valid_action_types(self) -> dict:
        ...
        valid_op_shift = self._builder.validate_op(ShiftOperator)
        valid_op = (
            valid_op_unary
            or valid_op_binary
            or valid_op_rolling
