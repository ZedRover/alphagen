...
from alphagen.data.expression_ocean import OCOperators
from alphagen.data.tokens import *


Operators = Operators + OCOperators  # NOTE add OCOperators to Operators
...
class TokenEmbedding(nn.Module):
    def __init__(
        ...
        total_emb = (
            len(SequenceIndicatorType) + len(FeatureType) + len(Operators) + dt_count
        )
    ...
    def forward(self, tokens: List[Token]) -> Tensor:
        ...
        for i, tok in enumerate(tokens):
            ...
            if isinstance(tok, SequenceIndicatorToken):
                ...
                emb_type_idx.append(
                    int(tok.delta_time) - self._delta_time_range[0] + dt_offset
                )
            else:
                assert False, "NullToken is not allowed here"

        result = torch.zeros(
            len(tokens), self._d_model, dtype=torch.float, device=self._device
        )
        if len(const_idx) != 0:
            const_tensor = torch.tensor(consts, device=self._device).unsqueeze(1)
            result[const_idx] = self._const_linear(const_tensor)
        if len(emb_idx) != 0:
            result[emb_idx] = self._emb(
                torch.tensor(emb_type_idx, dtype=torch.long, device=self._device)
            )
...
class ExpressionGenerator(nn.Module):
    def __init__(
        ...
        self._dt_linear = nn.Linear(
            d_model, delta_time_range[1] - delta_time_range[0], device=device
        )
    ...
    def forward(
        ...
        res: Tensor = self._decoder(  # [L, 1, D]
            decoder_state.unsqueeze(1), encoder_state.unsqueeze(1), tgt_mask=causal_mask
        )
