...
def make_function(*, function, name, arity, wrap=True):
    ...
    if not isinstance(function, np.ufunc):
        if function.__code__.co_argcount != arity:
            raise ValueError(
                "arity %d does not match required number of "
                "function arguments of %d." % (arity, function.__code__.co_argcount)
            )
    ...
    try:
        ...
        raise ValueError(
            "supplied function %s does not support arity of %d." % (name, arity)
        )
    ...
    if function(*args).shape != (10,):
        raise ValueError(
            "supplied function %s does not return same shape as "
            "input vectors." % name
        )

    if wrap:
        return _Function(
            function=wrap_non_picklable_objects(function), name=name, arity=arity
        )
