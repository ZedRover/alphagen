...
def test_validate_fitness():
    ...
    assert_raises(
        ValueError, make_fitness, function=_mean_square_error, greater_is_better="Sure"
    )
    assert_raises(
        ValueError, make_fitness, function=_mean_square_error, greater_is_better=1
    )
    # non-bool wrap
    assert_raises(
        ValueError,
    ...
    assert_raises(ValueError, make_fitness, function=bad_fun1, greater_is_better=True)

    # Check return type tests
    def bad_fun2(x1, x2, w):
        return "ni"

    assert_raises(ValueError, make_fitness, function=bad_fun2, greater_is_better=True)

    def _custom_metric(y, y_pred, w):
        """Calculate the root mean square error."""
        return np.sqrt(np.average(((y_pred - y) ** 2), weights=w))

    custom_metric = make_fitness(function=_custom_metric, greater_is_better=True)
...
def test_custom_regressor_metrics():
    ...
    def neg_mean_absolute_error(y, y_pred, sample_weight):
        return -1 * mean_absolute_error(y, y_pred, sample_weight=sample_weight)

    customized_fitness = make_fitness(
        function=neg_mean_absolute_error, greater_is_better=True
    )

    c_est_gp = SymbolicRegressor(
        metric=customized_fitness,
        stopping_criteria=-0.000001,
        random_state=415,
        parsimony_coefficient=0.001,
        verbose=0,
...
def test_custom_transformer_metrics():
    ...
    def _neg_weighted_pearson(y, y_pred, w):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            corr = (np.sum(w * y_pred_demean * y_demean) / np.sum(w)) / np.sqrt(
    ...
    neg_weighted_pearson = make_fitness(
        function=_neg_weighted_pearson, greater_is_better=False
    )

    c_est_gp = SymbolicTransformer(
        generations=2,
        population_size=100,
        hall_of_fame=10,
        n_components=1,
...
def test_custom_classifier_metrics():
    ...
    customized_fitness = make_fitness(
        function=negative_log_loss, greater_is_better=True
    )

    c_est_gp = SymbolicClassifier(
        metric=customized_fitness,
        stopping_criteria=0.000001,
        random_state=415,
        parsimony_coefficient=0.01,
...
def test_parallel_custom_metric():
    ...
    est = SymbolicRegressor(
        generations=2, metric=custom_metric, random_state=0, n_jobs=2
    )
    est.fit(diabetes.data, diabetes.target)
    _ = pickle.dumps(est)

    # Unwrapped functions should fail
    custom_metric = make_fitness(
        function=_custom_metric, greater_is_better=True, wrap=False
    )
    est = SymbolicRegressor(
        generations=2, metric=custom_metric, random_state=0, n_jobs=2
    )
    ...
    est = SymbolicRegressor(generations=2, metric=custom_metric, random_state=0)
    est.fit(diabetes.data, diabetes.target)
    assert_raises(AttributeError, pickle.dumps, est)
