...
def test_validate_function():
    ...
    # non-integer arity
    assert_raises(
        ValueError, make_function, function=_protected_sqrt, name="sqrt", arity="1"
    )
    assert_raises(
        ValueError, make_function, function=_protected_sqrt, name="sqrt", arity=1.0
    )
    # non-bool wrap
    assert_raises(
        ValueError,
    ...
    # non-matching arity
    assert_raises(
        ValueError, make_function, function=_protected_sqrt, name="sqrt", arity=2
    )
    assert_raises(ValueError, make_function, function=maximum, name="max", arity=1)

    # Check name test
    assert_raises(ValueError, make_function, function=_protected_sqrt, name=2, arity=1)

    # Check return type tests
    def bad_fun1(x1, x2):
        return "ni"

    assert_raises(ValueError, make_function, function=bad_fun1, name="ni", arity=2)

    # Check return shape tests
    def bad_fun2(x1):
        return np.ones((2, 1))

    assert_raises(ValueError, make_function, function=bad_fun2, name="ni", arity=1)
    ...
    assert_raises(
        ValueError, make_function, function=_unprotected_sqrt, name="sqrt", arity=1
    )
    ...
    assert_raises(
        ValueError, make_function, function=_unprotected_div, name="div", arity=2
    )
...
def test_parallel_custom_function():
    ...
    logical = make_function(function=_logical, name="logical", arity=4, wrap=False)
    est = SymbolicRegressor(
        generations=2,
        function_set=["add", "sub", "mul", "div", logical],
        random_state=0,
...
def test_parallel_custom_transformer():
    ...
    est = SymbolicClassifier(
        generations=2, transformer=sigmoid, random_state=0, n_jobs=2
    )
    ...
    # Single threaded will also fail in non-interactive sessions
    est = SymbolicClassifier(generations=2, transformer=sigmoid, random_state=0)
    est.fit(cancer.data, cancer.target)
    assert_raises(AttributeError, pickle.dumps, est)
