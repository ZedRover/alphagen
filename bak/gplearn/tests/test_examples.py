...
def test_symbolic_regressor():
    ...
    assert est_gp._program.parents == {
        "method": "Crossover",
        "parent_idx": 1555,
...
def test_symbolic_transformer():
    ...
    assert_almost_equal(
        est.score(diabetes.data[300:, :], diabetes.target[300:]),
        desired=0.43406,
        decimal=5,
    )
    ...
    assert_almost_equal(
        est.score(new_diabetes[300:, :], diabetes.target[300:]),
        desired=0.53368,
        decimal=5,
    )
...
def test_custom_functions():
    ...
    expected = "add(X3, logical(div(X5, sub(X5, X5)), " "add(X9, -0.621), X8, X4))"
...
def test_symbolic_classifier():
    ...
    est = SymbolicClassifier(
        parsimony_coefficient=0.01, feature_names=cancer.feature_names, random_state=1
    )
