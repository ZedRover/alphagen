...
from gplearn.functions import add2, sub2, mul2, div2, sqrt1, log1, abs1, max2, min2
...
def test_program_init_method():
    ...
    params = {
        ...
        "arities": {1: [sqrt1, log1, abs1], 2: [add2, sub2, mul2, div2, max2, min2]},
        "init_depth": (2, 6),
        "n_features": 10,
        "const_range": (-1.0, 1.0),
    ...
    for _ in range(20):
        programs.append(
            _Program(init_method="half and half", random_state=random_state, **params)
        )
...
def test_all_metrics():
    ...
    expected = [1.48719809776, 1.82389179833, 1.76013763179, -0.2928200724, -0.5]
    result = []
...
def test_input_validation():
    ...
    for Symbolic in (SymbolicRegressor, SymbolicTransformer):
        ...
        est = Symbolic(p_point_mutation=0.5)
        assert_raises(ValueError, est.fit, diabetes.data, diabetes.target)
...
def test_input_validation_classifier():
    ...
    # Check too much proba
    est = SymbolicClassifier(p_point_mutation=0.5)
    ...
    assert_raises(ValueError, est.fit, cancer.data, cancer.target)
    # And check acceptable, but strange, representations of const_range
    est = SymbolicClassifier(population_size=100, generations=1, const_range=(2, 2))
    est.fit(cancer.data, cancer.target)
    est = SymbolicClassifier(population_size=100, generations=1, const_range=None)
    est.fit(cancer.data, cancer.target)
    est = SymbolicClassifier(population_size=100, generations=1, const_range=(4, 2))
    est.fit(cancer.data, cancer.target)
    ...
    assert_raises(ValueError, est.fit, cancer.data, cancer.target)
    # And check acceptable, but strange, representations of init_depth
    est = SymbolicClassifier(population_size=100, generations=1, init_depth=(2, 2))
    est.fit(cancer.data, cancer.target)
    ...
    for t in ["sigmoid"]:
        est = SymbolicClassifier(population_size=100, generations=1, transformer=t)
        est.fit(cancer.data, cancer.target)
...
def test_none_const_range():
    ...
    est = SymbolicRegressor(population_size=100, generations=2, const_range=None)
    est.fit(diabetes.data, diabetes.target)
    float_count = 0
...
def test_sample_weight_and_class_weight():
    ...
    est1 = SymbolicRegressor(population_size=100, generations=2, random_state=0)
    est1.fit(diabetes.data, diabetes.target)
    est2 = SymbolicRegressor(population_size=100, generations=2, random_state=0)
    est2.fit(diabetes.data, diabetes.target, sample_weight=sample_weight)
    # And again with a scaled sample_weight
    est3 = SymbolicRegressor(population_size=100, generations=2, random_state=0)
    ...
    est1 = SymbolicClassifier(population_size=100, generations=2, random_state=0)
    est1.fit(cancer.data, cancer.target)
    est2 = SymbolicClassifier(population_size=100, generations=2, random_state=0)
    est2.fit(cancer.data, cancer.target, sample_weight=sample_weight)
    # And again with a scaled sample_weight
    est3 = SymbolicClassifier(population_size=100, generations=2, random_state=0)
    est3.fit(cancer.data, cancer.target, sample_weight=sample_weight * 1.1)
    # And then using class weight to do the same thing
    est4 = SymbolicClassifier(
        class_weight={0: 1, 1: 1}, population_size=100, generations=2, random_state=0
    )
    est4.fit(cancer.data, cancer.target)
    est5 = SymbolicClassifier(
    ...
    sample_weight = np.ones(diabetes.target.shape[0])
    est1 = SymbolicTransformer(population_size=100, generations=2, random_state=0)
    est1 = est1.fit_transform(diabetes.data, diabetes.target)
    est2 = SymbolicTransformer(population_size=100, generations=2, random_state=0)
    est2 = est2.fit_transform(
...
def test_trigonometric():
    ...
    est1 = mean_absolute_error(
        est1.predict(diabetes.data[400:, :]), diabetes.target[400:]
    )
    ...
    est2 = mean_absolute_error(
        est2.predict(diabetes.data[400:, :]), diabetes.target[400:]
    )
...
def test_subsample():
    ...
    est1 = SymbolicRegressor(
        population_size=100, generations=2, max_samples=1.0, random_state=0
    )
    est1.fit(diabetes.data[:400, :], diabetes.target[:400])
    est1 = mean_absolute_error(
        est1.predict(diabetes.data[400:, :]), diabetes.target[400:]
    ...
    est2 = SymbolicRegressor(
        population_size=100, generations=2, max_samples=0.1, random_state=0
    )
    est2.fit(diabetes.data[:400, :], diabetes.target[:400])
    est2 = mean_absolute_error(
        est2.predict(diabetes.data[400:, :]), diabetes.target[400:]
...
def test_parsimony_coefficient():
    ...
    est1 = SymbolicRegressor(
        population_size=100, generations=2, parsimony_coefficient=0.001, random_state=0
    )
    est1.fit(diabetes.data[:400, :], diabetes.target[:400])
    est1 = mean_absolute_error(
        est1.predict(diabetes.data[400:, :]), diabetes.target[400:]
    ...
    est2 = SymbolicRegressor(
        population_size=100, generations=2, parsimony_coefficient="auto", random_state=0
    )
    est2.fit(diabetes.data[:400, :], diabetes.target[:400])
    est2 = mean_absolute_error(
        est2.predict(diabetes.data[400:, :]), diabetes.target[400:]
...
def test_early_stopping():
    ...
    est1 = SymbolicRegressor(
        population_size=100, generations=2, stopping_criteria=200, random_state=0
    )
    est1.fit(diabetes.data[:400, :], diabetes.target[:400])
    assert len(est1._programs) == 1

    est1 = SymbolicTransformer(
        population_size=100, generations=2, stopping_criteria=100, random_state=0
    )
...
def test_verbose_output():
    ...
    est = SymbolicRegressor(
        population_size=100, generations=10, random_state=0, verbose=1
    )
    est.fit(diabetes.data, diabetes.target)
    ...
    true_header = "    |{:^25}|{:^42}|".format("Population Average", "Best Individual")
    ...
    true_header = line_format.format(
...
def test_verbose_with_oob():
    ...
    sys.stdout = StringIO()
    est = SymbolicRegressor(
        population_size=100, generations=10, max_samples=0.9, random_state=0, verbose=1
    )
    est.fit(diabetes.data, diabetes.target)
    verbose_output = sys.stdout
    sys.stdout = old_stdout
...
def test_more_verbose_output():
    ...
    sys.stderr = StringIO()
    est = SymbolicRegressor(
        population_size=100, generations=10, random_state=0, verbose=2
    )
    est.fit(diabetes.data, diabetes.target)
    verbose_output = sys.stdout
...
def test_parallel_train():
    ...
    ests = [
        SymbolicRegressor(
            population_size=100, generations=4, n_jobs=n_jobs, random_state=0
        ).fit(diabetes.data[:100, :], diabetes.target[:100])
    ...
    ests = [
        SymbolicClassifier(
            population_size=100, generations=4, n_jobs=n_jobs, random_state=0
        ).fit(cancer.data[:100, :], cancer.target[:100])
        for n_jobs in [1, 2, 3, 8, 16]
    ]
...
def test_pickle():
    ...
    # Check the transformer
    est = SymbolicTransformer(population_size=100, generations=2, random_state=0)
    est.fit(diabetes.data[:100, :], diabetes.target[:100])
...
def test_output_shape():
    ...
    est = SymbolicTransformer(
        population_size=100, generations=2, n_components=5, random_state=0
    )
    est.fit(X, y)
...
def test_gridsearch():
    ...
    clf = SymbolicRegressor(
        population_size=50, generations=5, tournament_size=5, random_state=0
    )
    grid = GridSearchCV(clf, parameters, cv=3, scoring="neg_mean_absolute_error")
...
def test_pipeline():
    ...
    est = make_pipeline(
        StandardScaler(),
        SymbolicRegressor(
            population_size=50, generations=10, tournament_size=5, random_state=0
        ),
    )
    est.fit(diabetes.data, diabetes.target)
    assert_almost_equal(
        est.score(diabetes.data, diabetes.target), -3.702070228336284, decimal=5
    )
...
def test_transformer_iterable():
    ...
    est = SymbolicTransformer(
        population_size=500, generations=2, function_set=function_set, random_state=0
    )
...
def test_validate_functions():
    ...
    for Symbolic in (SymbolicRegressor, SymbolicTransformer):
        ...
        # These should fail
        est = Symbolic(
            generations=2, random_state=0, function_set=("ni", "sub", "mul", div2)
        )
        assert_raises(ValueError, est.fit, diabetes.data, diabetes.target)
        est = Symbolic(
            generations=2, random_state=0, function_set=(7, "sub", "mul", div2)
        )
        assert_raises(ValueError, est.fit, diabetes.data, diabetes.target)
...
def test_low_memory():
    ...
    est = SymbolicRegressor(
        population_size=50, generations=10, random_state=56, low_memory=True
    )
...
def test_low_memory_warm_start():
    ...
    est = SymbolicRegressor(
        population_size=50, generations=20, random_state=415, low_memory=True
    )
    est.fit(diabetes.data, diabetes.target)
    cold_fitness = est._program.fitness_
    cold_program = est._program.__str__()
    ...
    est = SymbolicRegressor(
        population_size=50, generations=10, random_state=415, low_memory=True
    )
    est.fit(diabetes.data, diabetes.target)
