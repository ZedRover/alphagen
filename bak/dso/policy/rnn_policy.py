...
class RNNPolicy(Policy):
    ...
    def _setup_tf_model(
        ...
        with tf.name_scope("controller"):
            ...
            def loop_fn(time, cell_output, cell_state, loop_state):
                if cell_output is None:  # time == 0
                    ...
                    action = tf.random.categorical(
                        logits=logits, num_samples=1, dtype=tf.int32, seed=1
                    )[:, 0]
                    ...
                    next_obs, next_prior, next_finished = tf.py_func(
            ...
            memory_neglogp, _ = self.make_neglogp_and_entropy(
                self.memory_batch_ph, None
            )
    ...
    def make_neglogp_and_entropy(self, B, entropy_gamma):
        ...
        entropy_gamma_decay = np.array(
            [entropy_gamma**t for t in range(self.max_length)], dtype=np.float32
        )
        ...
        actions_one_hot = tf.one_hot(
            B.actions, depth=self.n_choices, axis=-1, dtype=tf.float32
        )
        ...
        entropy_gamma_decay_mask = (
            sliced_entropy_gamma_decay * mask
