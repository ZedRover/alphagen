...
class StateChecker(Token):
    ...
    def function(self, value_if_true, value_if_false):
        ...
        ), "StateChecker.state_value has not been set."
        return np.where(
...
class MultiDiscreteAction(Token):
    ...
    def __init__(self, value, action_dim=None):
        ...
        if action_dim is None:
            ...
            name = "STOP"
            super().__init__(
                function=self.apply_action, name=name, arity=0, complexity=1
            )
        else:
            assert isinstance(value, int) and value >= 0
            name = "a{}_{}".format(action_dim + 1, value + 1)
            super().__init__(
                function=self.apply_action, name=name, arity=1, complexity=1
            )
...
class Library:
    ...
    def __init__(self, tokens):
        ...
        self.multi_discrete_tokens = np.array(
            [
                ...
                if isinstance(t, MultiDiscreteAction)
            ],
            dtype=np.int32,
        )
    ...
    def tokenize(self, inputs):
        ...
        if isinstance(inputs, str):
            inputs = inputs.split(",")
        elif not isinstance(inputs, list) and not isinstance(
            inputs, np.ndarray
        ):  # TBD FIX HACK
            inputs = [inputs]
        tokens = [
