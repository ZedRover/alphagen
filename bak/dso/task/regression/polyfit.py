...
class PolyRegressorMixin:
    ...
    def np_array_signature(self, X):
        ...
        return hash(
            (
                ...
                X.diagonal(offset=X.shape[1] - X.shape[0]).tobytes(),
            )
        )  # lowest diagonal
    ...
    def zero_out_ls_terms(self, cLS, XtX_inv, zero_out_indices):
        ...
        z = scipy.linalg.solve(
            E, cLS[zero_out_indices], assume_a="pos"
        )  # take advantage D is PD
...
class DSOLeastSquaresRegressor(PolyRegressorMixin):
    ...
    def __init__(self, cutoff_p_value=1.0, n_max_terms=None, coef_tol=1e-12):
        ...
        self.data_dict = {}
        if (
            isinstance(cutoff_p_value, float)
            and cutoff_p_value > 0.0
            and cutoff_p_value <= 1.0
        ):
    ...
    def fit(self, X, y, X_signature=None):
        ...
        if self.cutoff_p_value_ < 1.0 or (
            ...
            and np.count_nonzero(np.abs(self.coef_) > self.coef_tol_)
            > self.n_max_terms_
        ):
            ...
            n_terms = next(
                (
                    x[0]
                    for x in enumerate(perm)
                    if p_values[x[1]] > self.cutoff_p_value_
                ),
                len(p_values),
            )
            ...
            if n_terms < len(self.coef_):
                ...
                self.coef_ = self.zero_out_ls_terms(
                    self.coef_, lsd.XtX_inv, zero_out_indices
                )
...
class DSOLassoRegressor(PolyRegressorMixin):
    ...
    def __init__(self, gamma=0.1, comp_tol=1e-4, rtrn_constrnd_ls=True):
        ...
        self.rtrn_constrnd_ls_ = (
            rtrn_constrnd_ls  # return re-optimized sparse least-squares
        )
        self.data_dict = {}
        self.n_max_records = 10
    ...
    def fit(self, X, y, X_signature=None):
        ...
        self.coef_ = self.dual_lasso(
            ldata.XtX_inv, ldata.X_pinv, ldata.n_obs, ldata.n_params, y
        )
    ...
    def dual_lasso(self, XtX_inv, X_pinv, n_obs, n_params, y):
        ...
        f_obj = lambda rho: 1 / 2 * np.dot(rho, np.matmul(XtX_inv, rho)) - np.dot(
            beta_LS, rho
        )
        ...
        bnds = scipy.optimize.Bounds(
            -rho_bnd * np.ones(n_params), rho_bnd * np.ones(n_params)
        )
        ...
        if self.rtrn_constrnd_ls_:
            ...
            zero_out_indices = [
                ...
                if 0.25 * (1 + rho_opt[i] / rho_bnd) * (1 - rho_opt[i] / rho_bnd)
                > self.comp_tol_
            ]
            ...
            beta_cLS[
                zero_out_indices
            ] = 0.0  # these will be zero up to floating point precision
            return beta_cLS
        else:
            ...
            for i in range(n_params):
                if (
                    0.25 * (1 + rho_opt[i] / rho_bnd) * (1 - rho_opt[i] / rho_bnd)
                    > self.comp_tol_
                ):
                    beta_Lasso[i] = 0.0
            # return solution
...
def nonnegative_int_tuples_to_sum(length, given_sum):
    ...
    if length == 1:
        ...
        for value in range(given_sum + 1):
            for permutation in nonnegative_int_tuples_to_sum(
                length - 1, given_sum - value
            ):
...
class PolyOptimizer(PolyRegressorMixin):
    ...
    def fit(self, X, y):
        ...
        if np.isfinite(self.regressor.coef_).all():
            ...
            return Polynomial(
                list(compress(pod.all_exponents, mask)), self.regressor.coef_[mask]
            )
...
class PolyGenerator(object):
    ...
    def generate(
        self, n_terms_mean=2, n_terms_sd=1, coef_mean=0, coef_sd=10, coef_precision=2
    ):
        """
