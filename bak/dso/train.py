...
class Trainer:
    def __init__(
        ...
        if self.use_memory:
            assert (
                self.epsilon is not None and self.epsilon < 1.0
            ), "Memory queue is only used with risk-seeking."
            self.memory_queue = make_queue(
                policy=self.policy, priority=False, capacity=int(memory_capacity)
            )
    ...
    def run_one_step(self, override=None):
        ...
        if self.gp_controller is not None:
            deap_programs, deap_actions, deap_obs, deap_priors = self.gp_controller(
                actions
            )
            ...
            if actions.shape[1] < deap_actions.shape[1]:
                ...
                actions, obs, priors = pad_action_obs_priors(
                    actions, obs, priors, pad_length
                )
                ...
                deap_actions, deap_obs, deap_priors = pad_action_obs_priors(
                    deap_actions, deap_obs, deap_priors, pad_length
                )
        ...
        if self.logger.save_top_samples_per_batch > 0:
            ...
            top_perc = int(
                len(programs) * float(self.logger.save_top_samples_per_batch)
            )
            for idx in sorted_idx[:top_perc]:
                top_samples_per_batch.append(
                    [self.iteration, r[idx], repr(programs[idx])]
                )
        ...
        if self.epsilon is not None and self.epsilon < 1.0:
            ...
            if self.use_memory:  # Memory-augmented quantile
                ...
                unique_programs = [
                    p for p in programs if p.str not in self.memory_queue.unique_items
                ]
                ...
                quantile = weighted_quantile(
                    values=combined_r, weights=combined_w, q=1 - self.epsilon
                )
        ...
        if self.baseline == "ewma_R":
            ewma = (
                np.mean(r)
                if ewma is None
                else self.alpha * np.mean(r) + (1 - self.alpha) * ewma
            )
            b = ewma
            ...
            ewma = (
                np.min(r)
                if ewma is None
                else self.alpha * quantile + (1 - self.alpha) * ewma
            )
            b = ewma
            ...
            ewma = (
                np.mean(r) - quantile
                if ewma is None
                else self.alpha * (np.mean(r) - quantile) + (1 - self.alpha) * ewma
            )
            b = quantile + ewma

        # Compute sequence lengths
        lengths = np.array(
            [min(len(p.traversal), self.policy.max_length) for p in programs],
            dtype=np.int32,
        )
        ...
        if self.priority_queue is not None:
            ...
            pqt_batch = self.priority_queue.sample_batch(
                self.policy_optimizer.pqt_batch_size
            )
            # Train the policy
        ...
        if r_max > self.r_best:
            ...
            if self.verbose or self.debug:
                print(
                    "[{}] Training iteration {}, current best R: {:.4f}".format(
    ...
    def save(self, save_path):
        ...
        state_dict = {
            ...
            "r_best": self.r_best,
            "p_r_best_tokens": self.p_r_best.tokens.tolist()
            if self.p_r_best is not None
            else None,
        }
