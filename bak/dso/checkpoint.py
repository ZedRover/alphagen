...
class Checkpoint:
    ...
    def __init__(
        self, model, load_path=None, save_freq=23, units="hours", save_on_done=False
    ):
        """
    ...
    def update(self):
        ...
        # Save if iteration-based frequency is met
            ...
            self.save_freq_iters is not None
            and (self.model.trainer.iteration % self.save_freq_iters) == 0
        ):
            self.save()
    ...
    def save(self, save_path=None):
        ...
        if save_path is None:
            ...
            ), "Cannot support automated checkpointing with model.save_dir=None."
            timestamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
            save_path = os.path.join(
                self.checkpoint_dir, "checkpoint_{}".format(timestamp)
            )
        if os.path.exists(save_path):
            paths = os.listdir(os.path.dirname(save_path))
            paths = [
                path for path in paths if path.startswith(os.path.basename(save_path))
            ]
    ...
    def load(self, load_path):
        ...
        programs = [
            from_tokens(np.array(tokens, dtype=np.int32))
            for tokens in cache_df["tokens"]
        ]
