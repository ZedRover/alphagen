...
def make_policy_optimizer(
    sess, policy, policy_optimizer_type, **config_policy_optimizer
):
    ...
    if policy_optimizer_type == "pg":
        ...
        assert issubclass(
            policy_optimizer_class, Policy
        ), "Custom policy {} must subclass dso.policy.Policy.".format(
            policy_optimizer_class
        )

    policy_optimizer = policy_optimizer_class(sess, policy, **config_policy_optimizer)
...
class PolicyOptimizer(ABC):
    ...
    def _init_loss_with_entropy(self) -> None:
        ...
        with tf.name_scope("losses"):
            self.neglogp, entropy = self.policy.make_neglogp_and_entropy(
                self.sampled_batch_ph, self.entropy_gamma
            )

            # Entropy loss
            self.entropy_loss = -self.entropy_weight * tf.reduce_mean(
    ...
    def _setup_optimizer(self):
        ...
        optimizer = make_optimizer(
            name=self.optimizer, learning_rate=self.learning_rate
        )
    ...
    @abstractmethod
    def train_step(self, baseline: np.ndarray, sampled_batch: Batch) -> summaries:
