...
class PQTPolicyOptimizer(PolicyOptimizer):
    ...
    def _set_loss(self):
        with tf.name_scope("losses"):
            ...
            self.pqt_batch_ph = make_batch_ph(
                "pqt_batch", self.n_choices
            )  # self.n_choices is defined in parent class

            pqt_neglogp, _ = self.policy.make_neglogp_and_entropy(
                self.pqt_batch_ph, self.entropy_gamma
            )
            self.pqt_loss = self.pqt_weight * tf.reduce_mean(
                pqt_neglogp, name="pqt_loss"
            )
