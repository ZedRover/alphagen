...
class PPOPolicyOptimizer(PolicyOptimizer):
    ...
    def _set_loss(self):
        with tf.name_scope("losses"):
            ...
            self.old_neglogp_ph = tf.placeholder(
                dtype=tf.float32, shape=(None,), name="old_neglogp"
            )
            ...
            ppo_loss = -tf.reduce_mean(
                (r - self.baseline) * tf.minimum(ratio, clipped_ratio)
            )
            ...
            clipped = tf.logical_or(
                ratio < (1.0 - self.ppo_clip_ratio), ratio > 1.0 + self.ppo_clip_ratio
            )
        ...
        for ppo_iter in range(self.ppo_n_iters):
            ...
            for i, mb in enumerate(minibatches):
                ...
                summaries, _ = self.sess.run(
                    [self.summaries, self.train_op], feed_dict=mb_feed_dict
                )
