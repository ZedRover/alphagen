...
def get_base_config(task, language_prior):
    # Load base config
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "config_common.json"),
        encoding="utf-8",
    ) as f:
    ...
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), task_config_file),
        encoding="utf-8",
    ) as f:
    ...
    if language_prior:
        with open(
            os.path.join(
                os.path.dirname(os.path.realpath(__file__)), "config_language.json"
            ),
            encoding="utf-8",
        ) as f:
...
def load_config(config=None):
    ...
    try:
        ...
        task = "regression"
        print(
            "WARNING: Task type not specified. Falling back to default task type '{}' to load config.".format(
                task
            )
        )
    try:
