            ...
            for i, collision in enumerate(collisions):
                ...
                for j, ind_prior in enumerate(ind_priors):
                    if np.any(np.isneginf(ind_prior[collision])):
                        ...
                        row = [
                            "X" if x == -np.inf else "" for x in ind_prior[collision]
                        ]
...
class RelationalConstraint(Constraint):
    ...
    def __call__(self, actions, parent, sibling, dangling):
        if self.relationship == "descendant":
            ...
            mask += np.logical_and(
                np.isin(sibling, self.targets), np.isin(parent, self.adj_effectors)
            )
            prior = self.make_constraint(mask, self.targets)
...
class DomainRangeConstraint(Constraint):
    ...
    def __init__(self, library):
        ...
        domains = np.array(
            [domains[t.name] for t in self.library.tokens], dtype=np.float32
        )
        ranges = np.array(
            [ranges[t.name] for t in self.library.tokens], dtype=np.float32
        )
        ...
        for i in range(self.library.n_input_tokens):
            ...
            self.constraining_parents[i] = np.array(
                self.constraining_parents[i], dtype=np.int32
            )
    ...
    def __init__(self, library):
        ...
        for parent in library.state_checker_tokens:
            ...
            if len(targets) > 0:
                ...
                prior = RelationalConstraint(
                    library, targets=targets, effectors=effectors, relationship="lchild"
                )
                self.priors.append(prior)
            ...
            if len(targets) > 0:
                ...
                prior = RelationalConstraint(
                    library, targets=targets, effectors=effectors, relationship="rchild"
                )
                self.priors.append(prior)

        # Add priors that constraint any StateChecker from being a child of any non-StateChecker
        if len(library.state_checker_tokens) > 0:
            non_state_checker_tokens = np.setdiff1d(
                np.arange(self.L), library.state_checker_tokens, assume_unique=True
            )
...
class PolyConstraint(Constraint):
    ...
    def __init__(self, library):
        ...
        invalid_ancestors = np.intersect1d(
            invalid_ancestors, library.names, assume_unique=True
        )
        if len(invalid_ancestors) > 0:
            descendant_prior = RelationalConstraint(
                library,
                targets=["poly"],
        ...
        if library.const_token is not None:
            mutually_exclusive_tokens = np.array(
                [library.poly_token, library.const_token]
            )
            self.priors.append(
                MutuallyExclusiveConstraint(library, mutually_exclusive_tokens)
            )
...
class MultiDiscreteConstraint(Constraint):
    ...
    def __init__(self, library, dense, ordered):
        ...
        non_multi_discrete = np.setdiff1d(
            np.arange(self.L), library.multi_discrete_tokens, assume_unique=True
        ).tolist()

        self.unary_multi_discrete = np.intersect1d(
            library.multi_discrete_tokens, library.unary_tokens, assume_unique=True
        )
        ...
        if dense and ordered:
            ...
            for parent in self.unary_multi_discrete:
                if self.library[parent].action_dim < MultiDiscreteAction.n_dims - 1:
                    targets = [
                        t
                        for t in library.multi_discrete_tokens
                        if self.library[t].action_dim is None
                        or self.library[t].action_dim
                        != self.library[parent].action_dim + 1
                    ]
                else:
                    targets = self.unary_multi_discrete
                prior = RelationalConstraint(
                    library, targets=targets, effectors=[parent], relationship="child"
                )
                self.priors.append(prior)
            ...
            for parent in self.unary_multi_discrete:
                targets = [
                    t
                    for t in self.unary_multi_discrete
            ...
            self.special_prior = RelationalConstraint(
                library,
                targets=self.unary_multi_discrete,
    ...
    def describe(self):
        ...
        if self.ordered:
            message.append(
                indent + "action_dim in each action branch must be ascending."
            )
        return "\n".join(message)
