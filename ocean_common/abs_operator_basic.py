...
class OPTOOL:
    ...
    def __init__(self, uInstrNum=6000, diNum=99, tiNum=16, n_sample=99, dftp_s=99, dftp_l=99, fill='nofill', industry=np.array([]), marketsize=np.array([]), normd=21, eps=1e-5, correps = 1e-5, adigit=7, timeperiod=120, sharedstyle=False, sharedpath=None, needpatch = True, mode='single'):
    ...
    def crsrank(self, x):
        ...
        return (bk.nanrankdata(nx, axis=1)-(validnum+1)/2).reshape(-1, )
    ...
    def normcrsrank(self, x):
        ...
        nx = np.round(x, 6).reshape(-1, self.uInstrNum)
        validnum = np.isfinite(nx).sum(axis=1, keepdims=True)
        return (bk.nanrankdata(nx, axis=1)/validnum-0.5).reshape(-1, )
    ...
    def fillinf(func):
        ...
        def fillfunc(*args, **kwargs):
            mask = np.array([np.isfinite(ag) for ag in args if isinstance(ag, np.ndarray)]).all(axis=0)
    ...
    def filterinf(func):
        @wraps(func)
        def fillfunc(*args, **kwargs):
            args = [np.where(np.isinf(i), np.nan, i) if isinstance(i, np.ndarray) else i for i in args]
            x = func(*args, **kwargs)
            return x
        return fillfunc
    ...
    def crsstd(self,x):
        ...
        res = self.div((res-np.nanmean(res, axis = 1, keepdims=True)), np.nanstd(res, axis=1, keepdims=True))
    ...
    def capcrsstd(self, x, capthres=5):
        ...
        x = self.div((x-np.nanmean(x, axis=1, keepdims=True)), np.nanstd(x, axis=1, keepdims=True))
        x = np.where(mask, x, (sign>0)*np.nanmax(x, axis=1, keepdims=True)+(sign<0)*np.nanmin(x, axis=1, keepdims=True))
        return x.reshape(-1, )
    ...
    def crsabs(self,x):
        ...
        res = x.reshape((-1,self.uInstrNum))
        res = (res-np.nanmean(res, axis = 1, keepdims=True))/np.nansum(np.abs(res), axis=1, keepdims=True)
        return self.fillnan(res.reshape((-1, )))
    ...
    def tick_lineardecay(self, x, tick = 5):
        ...
        tick_ldecay_mat = np.array([ [[i]*self.uInstrNum]*int(x.shape[0]/self.uInstrNum) for i in range(tick, 0, -1)])

        nx = x.reshape((-1, self.uInstrNum))
        return (np.sum(tick_ldecay_mat*nx, axis=0)/np.sum(list(range(1,tick+1)))).reshape((-1,))
    ...
    def TSfunc(self, func, x, tick = 5):
        ...
        res = x.reshape((-1, self.uInstrNum))
        res = np.stack([self.roll_np(res, shift = i, axis=0).reshape((-1,)) for i in range(tick)])
    ...
    def tick_tsmax_single(self, x, tick = 5):
        x = x.reshape(-1, self.uInstrNum)
        return bk.move_max(x, tick, min_count=1, axis=0).reshape(-1,)

    @fillinf
    def tick_tsmin_single(self, x, tick = 5):
        x = x.reshape(-1, self.uInstrNum)
        return bk.move_min(x, tick, min_count=1, axis=0).reshape(-1,)

    @fillinf
    def tick_tsargmax_single(self, x, tick = 5):
        x = np.round(x, 5).reshape(-1, self.uInstrNum)
        return bk.move_argmax(x, tick, min_count=1, axis=0).reshape(-1,)

    @fillinf
    def tick_tsargmin_single(self, x, tick = 5):
        x = np.round(x, 5).reshape(-1, self.uInstrNum)
        return bk.move_argmin(x, tick, min_count=1, axis=0).reshape(-1,)


    @fillinf
    def tick_tsstd_single(self, x, tick = 5):
        x = x.reshape(-1, self.uInstrNum)
        res = bk.move_std(x, tick, min_count=1, axis=0).reshape(-1,)
        if self.needpatch:
    ...
    def tick_tsmean_single(self, x, tick = 5):
        x = x.reshape(-1, self.uInstrNum)
        res = bk.move_mean(x, tick, min_count=1, axis=0).reshape(-1,)
        if self.needpatch:
            res = np.around(res, self.adigit)
        return res
    ...
    def tick_tssum_single(self, x, tick = 5):
        x = x.reshape(-1, self.uInstrNum)
        res = bk.move_sum(x, tick, min_count=1, axis=0).reshape(-1,)
        if self.needpatch:
            res = np.around(res, self.adigit)
        return res
    ...
    def tick_tsrank_single(self, x, tick = 5):
        ...
        return tick/2*bk.move_rank(x, tick, min_count=1, axis=0).reshape(-1,)
    ...
    def crscossimic(self, y, y_pred):
        ...
        iclist = (np.nansum(y*y_pred, axis = 1)/np.sqrt(np.nansum(y*y, axis = 1)*np.nansum(y_pred*y_pred, axis = 1)))
    ...
    def betacossimic(self, y, y_pred):
        return self.olcossimic(y.reshape((-1, self.uInstrNum))[:, 0], np.nansum(y_pred.reshape((-1, self.uInstrNum)), axis = 1))
    ...
    def TSfunc_2(self, func, x, y, tick = 5):
        ...
        stack1 = np.stack([self.roll_np(stack1, shift = i, axis=0).reshape((-1,)) for i in range(tick)])
        
        stack2 = y.reshape((-1, self.uInstrNum))
        stack2 = np.stack([self.roll_np(stack2, shift = i, axis=0).reshape((-1,)) for i in range(tick)])
    ...
    def tick_cov_single(self, x, y, tick = 5):
        ...
        x = x.reshape(-1, self.uInstrNum)
        y = y.reshape(-1, self.uInstrNum)
        cov = (bk.move_mean(x*y, tick, min_count=1, axis=0)-bk.move_mean(x, tick, min_count=1, axis=0)*bk.move_mean(y, tick, min_count=1, axis=0))
    ...
    #Not Safe.
    def mynorm(self, x):
        return np.apply_along_axis(self.pn_norm, 0, x.reshape((-1, self.uInstrNum))).reshape((-1, ))
    #Safe
    ...
    def fdnorm(self, x):
        ...
        normres = np.linalg.norm(np.where(np.isfinite(normrawdata), normrawdata, 0), axis = 0)/np.sqrt(validnum)
    ...
    def lmnorm(self, x, cachemode = False):
        ...
        norm = np.broadcast_to(np.sqrt(self.div(np.nansum(rx*rx, axis = 1, keepdims=True), np.sum(np.isfinite(rx), axis = 1, keepdims=True))), rx.shape).reshape((-1, ))
        tail = np.broadcast_to(norm[-self.uInstrNum:], (allticks%ticks, self.uInstrNum)).reshape((-1, ))
        norm = np.append(norm, tail)
        norm = self.roll_np(norm.reshape((-1, self.uInstrNum)), axis = 0, shift = ticks).reshape((-1, ))
    ...
    def rollingnorm(self, x, tick = 50):
        res = x.reshape((-1, self.uInstrNum))
        res = np.stack([np.roll(res, shift = i, axis=0).reshape((-1,)) for i in range(tick)])
    ...
    def interval_baseop(self, x, interval, func):
        ...
        cand = func(x[:totalslot-lap].reshape(-1, interval, self.uInstrNum)).reshape(-1, self.uInstrNum)
        cand = np.concatenate([np.full((interval, self.uInstrNum), np.nan), cand[:totalslot-interval]], axis=0)
    ...
    def cum_baseop(self, x, func):
        ...
        cand = func(x[:totalslot-lap].reshape(-1, self.tiNum, self.uInstrNum)).reshape(-1, self.uInstrNum)
        if lap > 0:
            cand2 = func(x[None, totalslot-lap:]).reshape(-1, self.uInstrNum)
            cand = np.concatenate([cand, cand2], axis=0)
        return cand.reshape(-1, )

    def cum_meanfunc(self, x, ):
        tinum = x.shape[1]
    ...
    def minmax_std(self, x):
        ...
        return self.div((x-xmin), (xmax-xmin)).reshape(-1, )